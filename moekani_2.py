...
from deck_structure import deck, get_fields_mapping, model
...
wanikani_vocab_cards = list(
    filter(lambda card: card["Type"] == "vocab", wanikani_mapped_data)
)
wanikani_kanji_cards = list(
    filter(lambda card: card["Type"] == "kanji", wanikani_mapped_data)
)
wanikani_radical_cards = list(
    filter(lambda card: card["Type"] == "radical", wanikani_mapped_data)
)
# first_thirty_radical_cards = wanikani_radical_cards[:30]
# wanikani_radical_cards = wanikani_radical_cards[30:]
...
for wanikani_card in wanikani_vocab_cards:
    ...
    for moe_card_index, moe_card in enumerate(moe_cards):
        ...
        pattern = (
            rf"(?:[^\u4E00-\u9FFF]|^){re.escape(trimmed_vocab)}(?:[^\u4E00-\u9FFF]|$)"
        )
...
for radical_card in wanikani_radical_cards:
    earliest_kanji_containing_radical = next(
        (
            kanji
            for kanji in wanikani_kanji_cards
            if radical_card["Radical"] in kanji["Radicals"]
        ),
        None,
    )
    ...
    if earliest_kanji_containing_radical:
        ...
        # Some radical meanings reference other radicals but there is no field for this, so adding all lower level radicals
        lower_level_radicals = filter(
            lambda card: int(card["Wanikani_Level"])
            < int(radical_card["Wanikani_Level"]),
            wanikani_radical_cards,
        )

        for lower_level_radical in lower_level_radicals:
            lower_level_radical["Moe_Index"] = min(
                earliest_kanji_containing_radical["Moe_Index"],
                lower_level_radical["Moe_Index"],
            )

        radical_card["Moe_Index"] = min(
            earliest_kanji_containing_radical["Moe_Index"], radical_card["Moe_Index"]
        )
...
for index, moe_card in enumerate(moe_cards):
    moe_card_radicals = filter(
        lambda card: card["Moe_Index"] == index, wanikani_radical_cards
    )
